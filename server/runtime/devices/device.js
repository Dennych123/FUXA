...
function Device(data, runtime) {
    ...
    else if (data.type === DeviceEnum.Fins) {
        if (!FinsClient) {
            return null;
        }
        comm = FinsClient.create(data, logger, events, manager, runtime);
    }
...
function loadPlugin(type, module) {
    ...
    } else if (type === DeviceEnum.Fins) {
        FinsClient = require(module);
    }
}
